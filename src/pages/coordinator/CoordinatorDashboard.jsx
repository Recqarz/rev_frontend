...
import { getCoordinatorCaseData } from "../../redux/dashboard/coordinator/coordinator.Action";
...
const CoordinatorDashboard = () => {
  ...
  const {data, isLoading} = useSelector((store) => store?.coordinatorReducer);
...
const coordinatorDashboard = [
    {
      ...
      link: "/coordinator/all/fieldexecutives",
    },

    {
      ...
      link: "/coordinator/all/cases",
    },
    {
      id: 6,
      name: "Banks",
      count: data?.allList?.banks?.totalBanks|| 0,
...
useEffect(()=>{
  dispatch(getCoordinatorCaseData(accessToken));
},[accessToken]);


  return (
    <div className="flex flex-col gap-10">
    <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-14">
      {coordinatorDashboard?.map((item) => (
